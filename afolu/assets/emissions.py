...
@dg.asset(
    name="transitions_emissions",
    key_prefix="small",
    ins={
        "df_out": dg.AssetIn(["small", "emissions"]),
    },
    partitions_def=wanted_zones_partitions,
    io_manager_key="dataframe_manager",
    group_name="small_emissions",
)
def transitions_emissions(df_out: pd.DataFrame) -> pd.DataFrame:
    _, matt, model_afolu, _, _ = generate_model_objects()

    modvar = get_modvar(matt, model_afolu.modvar_lndu_emissions_conv)
    df_vars = matt.extract_model_variable(df_out, modvar)
